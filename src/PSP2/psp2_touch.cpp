...
extern bool psp_rear_touch;
extern bool psp_indirect_touch;
extern float psp_pointer_speed_factor;
extern int32 psp_screen_x;
...
void psp2PollTouch(void) {
	...
	for(int port = 0; port < SCE_TOUCH_PORT_MAX_NUM; port++) {
		if ((psp_indirect_touch && port == SCE_TOUCH_PORT_FRONT) || (psp_rear_touch && port == SCE_TOUCH_PORT_BACK)) {
...
void psp2ConvertTouchXYToGameXY(float touchX, float touchY, int *gameX, int *gameY) {
	//map to display
	float x = touchX * 960.0;
	float y = touchY * 544.0;
	*gameX = (x - ((960.0 - psp_screen_x*scale_x)/2)) / scale_x;
	*gameY = y / scale_y;
}
